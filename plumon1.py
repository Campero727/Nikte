# -*- coding: utf-8 -*-
"""
Created on Thu Mar 11 22:49:26 2021
...
while True:
    ...
    #usamos el efecto espejo
    frame=cv2.flip(frame, 1)
    #convertir a HSV
    frameHSV=cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    
    #rellenamos la matriz con 0 de igual tamaño al frame
    if imAux is None: imAux = np.zeros(frame.shape,dtype=np.uint8)
    
    #seccion superior con los cuadritos para seleccionar color
    ...
    #controlamos el area de la tapa de color
    for c in cnts:
    ...
    #convertimos imAux a escala de Grises
    imAuxGray = cv2.cvtColor(imAux,cv2.COLOR_BGR2GRAY)
    #aplicamos una umbralizacion simple
